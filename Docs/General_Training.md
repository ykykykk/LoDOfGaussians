...
启动时估计训练图片解码后的大小。能放入不超过 8 GiB、且不超过当前可用内存 25% 的预算时，使用同进程共享 CPU 图像缓存，通过取图线程预取。这样避免 Windows 多进程反复传递整张浮点图像。Resident v2 的同进程缓存将图片锁页一次并在后续视角复用，省去每步重复锁页复制；锁页内存计入既有缓存预算。更大的数据保留分 worker 的有界 LRU 缓存。粗训练也复用解码缓存。
